...
final class EditProfileViewController: UIViewController, UITableViewDataSource {
    ...
    private func configureModels(){
        ...
        for label in section1Labels {
            let model = EditProfileFormModel(label: label, paceholder: "Enter \(label)...", value: nil)
...
extension EditProfileViewController: FormTableViewDelegate {
    func formTableViewCell(_ cell: FormTableViewCell, didUpdateFiled updateModel: EditProfileFormModel) {
        print("updated \(updateModel.label) : \(updateModel.value ?? "") ")
    }
